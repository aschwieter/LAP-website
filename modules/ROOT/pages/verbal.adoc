...
If an individual who is deaf or hard of hearing says that video remote interpreting isn't effective for them, federal law requires that we <<ed-inperson-spanish,request an in-person interpreter>>.
...
[[DIAL]]
NOTE: When families call us, they should use the language-specific https://centerlink.cchmc.org/department/interpreter-services/dial-direct-interpreter-access-line[Direct Intepreter Access Lines] (if available for their language) which connect with an interpreter and then the Cincinnati Children's operator. Avoid directing families to phone trees whenever possible since they are difficult to navigate with an interpreter on the line.

[.filter-home-health]
****
icon:house[] _Home Health_

When working in the community, if you phone calls are not working due to inadequate cell service, use a <<vide,video interpreter>> instead.
...
NOTE: For people who are deaf or hard of hearing and use American Sign Language (ASL), call the family directly as you would a family that speaks English. An ASL interpreter will connect automatically, communicating verbally with you and via video with the patient or family member.

=== Video [[video]]

Video interpreters are available on Alvins using the InSight app. You can search by language or country. Watch https://www.youtube.com/watch?v=6RXPzRH9jYQ[this video] for more details.
...
|
.LanguageLine Alvin
image::{imagesdir}/Interpreter-on-Wheels.jpg[iPad cart with white speakers on both sides of the iPad,100,100]]

| 
.Telehealth Alvin
image::{imagesdir}/telehealth-cart.jpg[iPad cart with a black speaker below the iPad,180,180]]
...
==== Troubleshooting
If calls repeatedly drop, switch to a <<phone,phone interpreter>> for spoken languages. For sign languages, call Language Access Services at 6-1444 (or the manager in Who's On Call after hours) for assistance.
...
[.filter-acute-critical-care.filter-ambulatory.filter-radiology.filter-periop.filter-mental-health]
****
icon:hospital[] _Acute/Critical Care_  icon:store[] _Ambulatory_  icon:radiation[] _Radiology_  icon:syringe[] _Periop_  icon:brain[] _Mental Health_

[discrete]

==== Spanish & Arabic at Base during Business Hours [[service-tasks]]
...
[.filter-acute-critical-care.filter-mental-health]
****
icon:hospital[] _Acute/Critical Care_  icon:brain[] _Inpatient/Residential Mental Health_
...
[.filter-ambulator.filter-periop.filter-home-health.filter-radiology.filter-mental-health]
****
icon:store[] _Ambulatory_  icon:syringe[] _Periop_  icon:house[] _Home Health_  icon:radiation[] _Radiology_  icon:brain[] _Outpatient Mental Health_
...
4. End of life
5. Home Health visits without cellular or WiFi coverage - please inform interpeterservices@cchmc.org if this is the case.
6. MRI
...
10. Rounds
11. Aquatic therapy
12. Shifts where interpreters are deployed on-demand (e.g. Emergency Department and <<service-tasks,Spanish & Arabic at Base during Business Hours>>)
...
Contact interpeterservices@cchmc.org if:
...
A list of QBS can be found https://cchmc.sharepoint.com/:l:/s/LanguageAccess/FGoEywo2hLdAvYC7YccczcwBLKMqG4lAm4T9vgyDfuwMLQ[here].
...
Form J1011 must be completed whenever an adult accompanying an individual with limited English proficiency interprets or facilitates communication. To find the form, search for "J1011" in https://cchmc.navexone.com/content/Search/Documents[PolicyTech].
...
Work with an interpreter when getting a patient from the waiting room if the patient or someone accompanying the patient would xref:languagepreference.adoc[prefer to communicate in a language other than English]. "Wrong patient" safety events can result from miscommunication in the waiting room.

== Groups, Community Events, and Conferences

It is complex to provide interpreting in large group settings such as community events and conferences. Email mailto:interpreterservices@cchmc.org[interpreterservices@cchmc.org] to arrange a consult with Language Access Services about your event as early as possible.
...
a|image::{imagesdir}/amn.png[AMN Interpreting Icon,width=50]
|===
...
For Remote Paitent Monitoring (RPM), use Teams instead of Vivify for calls where an interpreter is needed.
